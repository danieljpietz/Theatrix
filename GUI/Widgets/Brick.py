    ...
    def addPorts(self):
        ...
        for i in range(0, self.outputCount):
            ...
            port.move(self.initWidth - 1.5 * port.size().width(),
                      self.initHeight / 8 + i * 1.5 * (port.size().height()))
            port.portType = "Output"
            port.show()
            outputLabel = QLabel(self.outputs[i])
            outputLabel.setParent(self)
            outputLabel.adjustSize()
            outputLabel.move(self.initWidth - 1.5 * port.size().width() - outputLabel.width() - 10,
                             self.initHeight / 8 + i * 1.5 * (port.size().height()))
        ...
        if self.isSelected:
            painter.setBrush(
                QBrush(QColor(0, 0, 0, 0),
                       Qt.SolidPattern))
            painter.setPen(
                QPen(QColor(self.selectedColor[0], self.selectedColor[1], self.selectedColor[2], self.selectedColor[3]),
                     2, Qt.SolidLine))
